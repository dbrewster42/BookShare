...
class User(models.Model):
	...
	objects = UserManager()
	def __repr__(self):
		 return f"<User object: {self.first} ({self.id})>"
...
class BookManager(models.Manager):
	def basic_validator(self, dat):
		...
		if len(dat['author']) < 2:
			errors['author'] = "The man deserves credit!"
		if len(dat['desc']) < 5:
...
class FavManager(models.Manager):
	def basic_validator(self, dat):
		errors = {}
		if Favorite.objects.filter(user=data['user'], book=data['book']):
			errors['favorite'] = "You have already added this book to your favorites"
		return errors
...
class Favorite(models.Model):
	...
	objects = FavManager()
	def __repr__(self):
		return f"<Favorite object: {self.user.first} {self.book.title} ({self.id})>"
